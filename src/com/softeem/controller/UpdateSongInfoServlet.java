...
public class UpdateSongInfoServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            //上传成功修改上传的信息
            Song song = (Song) req.getSession().getAttribute("SONG_INFO");
            ...
            //将数据插入到数据库
            songServer.updateSongInfo(song);
            //跳回主页
            resp.sendRedirect(req.getContextPath() + "/toSongInfo.am");
        } catch (Exception e) {
            resp.getWriter().println("<h2>歌曲修改失败</h2>");
        } finally {
            if (is3 != null) {
                is3.close();
            }
